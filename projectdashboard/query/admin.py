...
def validate_fiscalyears_input(fiscal_years):
    for i, item in enumerate(fiscal_years):
        ...
        if item['start_date'] < fiscal_years[i-1]['end_date']:
            return False
        # Check start and end dates align with calendar quarters
        if item['start_date'][5:10] not in START_QUARTERS:
            return False
        if item['end_date'][5:10] not in END_QUARTERS:
            return False
    return True
...
def add_quarters(start_date, num_quarters):
    ...
    end_date = datetime.date(start_date.year, int(
        end_qtr_month), int(end_qtr_date))
...
def create_fiscal_periods_for_fiscal_year(fiscal_year):
    ...
    for order in range(1, fiscal_year.num_quarters+1):
        ...
        fiscal_period = create_fiscal_period_for_fiscal_year(
            fiscal_year, order, _start_date, _end_date)
...
def create_fiscal_years_for_choice(fiscal_year_choice, fiscal_years):
    ...
    i = 0
    while True:
        fiscal_year_start_date = datetime.date(
            start_date.year+i, start_date.month, start_date.day)
        fiscal_year = models.FiscalYear()
        fiscal_year.start = fiscal_year_start_date
        fiscal_year.end = add_quarters(
            fiscal_year_start_date, fiscal_year_choice.num_quarters)
        fiscal_year.id = make_fiscal_year_id(
            fiscal_year_start_date.year, fiscal_year.end.year, fiscal_year_choice.num_quarters)
        fiscal_year.name = fiscal_year.id
        fiscal_year.num_quarters = fiscal_year_choice.num_quarters
        fiscal_year.fiscal_year_periods = create_fiscal_periods_for_fiscal_year(
            fiscal_year)
        fiscal_years.append(fiscal_year)
        db.session.merge(fiscal_year)
        i += 1
        if fiscal_year.end == fiscal_year_choice.end_date:
            break
    db.session.commit()
    return fiscal_years
...
def assign_financial_data_to_fiscal_year(fiscal_year_ids):
    ...
    for finance in models.ActivityFinances.query.all():
        earliest_fps = [
            fp_date for fp_date in fps_start if fp_date <= finance.transaction_date]
    ...
    for forwardspend in models.ActivityForwardSpend.query.all():
        earliest_fps = [
            fp_date for fp_date in fps_start if fp_date <= forwardspend.period_start_date]
        if len(earliest_fps) == 0:
    ...
    for counterpartfunding in models.ActivityCounterpartFunding.query.all():
        earliest_fps = [
            fp_date for fp_date in fps_start if fp_date <= counterpartfunding.required_date]
        if len(earliest_fps) == 0:
...
def create_fiscal_years():
    ...
    for fiscal_year_choice in models.FiscalYearChoice.query.all():
        fiscal_years = create_fiscal_years_for_choice(
            fiscal_year_choice, fiscal_years)
