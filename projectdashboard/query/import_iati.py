...
def set_activity_iati_preferences(activity, iati_options):
    ...
    preferences = [pref for pref in activity.iati_preferences if pref.field in iati_options]
...
def import_data(activity_id, iati_identifier, activity_ids, import_options, activities_fields_options):
    ...
    activities = [qactivity.get_activity(_activity_id) for _activity_id in activity_ids]
    activities_lookup = dict((_activity.id, _activity) for _activity in activities)
    iati_activity = retrieve_data(iati_identifier)
    iati_activity_transactions = get_transactions_summary(iati_activity, False)
    iati_options = list(dict(filter(lambda option: option[1]=='IATI', import_options.items())).keys())
    ...
    activities_finances = [_finance for _activity in activities_lookup.values() for _finance in _activity.finances]
    activity_retained_finances = list(filter(filter_non_iati_transactions, activities_finances))
    ...
    def filter_iati_forwardspends(transaction):
        if transaction["transaction_type"] != 'budget': return False
        return True
    if 'forwardspend' in iati_options:
        iati_forwardspends = makeForwardSpendsFromTransactions(activity,
            aggregate_transactions(
                filter(filter_iati_forwardspends, iati_activity_transactions)
            )['forwardspend']['data'])
    ...
    activity.iati_preferences = set_activity_iati_preferences(activity, iati_options)
...
def makeFinanceFromTransaction(activity, transaction):
    ...
    f.transaction_type = iati_transaction_types.get(transaction['transaction_type'])
    ...
    if transaction['iati_identifier'].startswith('46002-'):
        ...
        if 'afdb' in nsmap:
            fund_source_code = transaction_xml.xpath('@afdb:finance-type-id', namespaces=nsmap)
            fund_source_name = transaction_xml.find("provider-org/narrative").text
            ...
            f.fund_source_id = get_or_create_fund_source(fund_source_name, finance_type, fund_source_code)
...
def aggregate_transactions(transactions):
    ...
    for transaction in transactions:
        grouped_transactions = group_transaction_data(transaction, grouped_transactions)
