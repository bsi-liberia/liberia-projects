...
def get_column_information(mtef, _headers):
    mtef_cols = list(filter(lambda col: re.match(
        r"(.*) \(MTEF\)", col), _headers))
    disb_cols = list(filter(lambda col: re.match(
        r"(.*) Q(.*) \(D\)", col), _headers))
    counterpart_funding_cols = list(filter(lambda col: re.match(
        r"(.*) \(GoL counterpart fund request\)", col), _headers))
