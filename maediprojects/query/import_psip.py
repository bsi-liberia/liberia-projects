    ...
    def append_path(root, data):
        ...
        if (float(data["REVISED_APPROPRIATION"]) != 0) or (float(data["ACTUAL"]) != 0):
            ...
            if float(data["ACTUAL"]) != 0:
                ...
                project["rows"].append(r)
            project["organisations"].add((data["ministry_code"], data["ministry_name"]))
            project["sectors"].add((data["sector_code"], data["sector_name"]))
            if data["fy"] < project["earliest_year"]: project["earliest_year"] = data["fy"]
            if data["fy"] > project["latest_year"]: project["latest_year"] = data["fy"]
            if data["fy"] == u"20182019": project["activity_status"] = "2"
    root = {}
    for row in csv: append_path(root, row)
    return root.values()
...
def process_transactions(activity, CODELIST_IDS_BY_CODE):
    ...
    for row in activity["rows"]:
        ...
        tr.transaction_description = u"{} {} FY{}, imported from IFMIS PSIP data".format(
            {u"C": "Revised appropriation for", u"D": "Actuals for"}[row["transaction_type"]],
            row["fiscalperiod"],
            row["fy"]
        )
        tr.transaction_value = row["transaction_value"]
        tr.currency = u"USD"
        tr.provider_org_id = clean_get_create_organisation(u"Government of Liberia")
        ...
        tr.classifications = process_transaction_classifications(row, CODELIST_IDS_BY_CODE)
