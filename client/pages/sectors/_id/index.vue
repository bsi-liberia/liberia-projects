<template>
  <div>
    ...
    <template v-else>
      ...
      <b-row>
        <b-col>
          <h2>Planned and Actual Disbursement by Donor</h2>
          <b-row>
            <b-col md="6">
              <b-form-group
                label="Fiscal year"
          ...
          <client-only>
            <sector-bar-chart
              ...
              agg-filter="mtef-sector"
              :agg-filter-value="sectorID"></sector-bar-chart>
          </client-only>
        </b-col>
      </b-row>
      <hr />
      <b-row>
        <b-col>
          <h2>Projected / Actual Disbursements by Donor, over time</h2>
          <b-row class="mb-2">
          ...
          <client-only>
            <sector-line-chart
              ...
              dimension="reporting-org"
              agg-filter="mtef-sector"
              :agg-filter-value="sectorID"></sector-line-chart>
          </client-only>
        </b-col>
      </b-row>
      <hr />
...
import SectorBarChart from '~/components/Sector/BarChart.vue'
import SectorLineChart from '~/components/Sector/LineChart.vue'
...
export default {
  ...
  head() {
    return {
      title: `Sector | ${this.$config.title}`
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    async getSector() {
      ...
      .then(response => {
        ...
        this.selectedFilters['mtef-sector'] = this.sector.id
        this.defaultFilters['mtef-sector'] = this.sector.id
      ...
      await this.$axios({url: `sector-brief/${this.sectorID}.docx`,
        ...
        if (response.status === 200) {
          saveAs(
            ...
            `${this.sector.name} Sector Brief.docx`
          )
